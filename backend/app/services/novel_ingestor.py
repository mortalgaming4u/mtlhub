# app/services/novel_ingestor.py
...
class NovelIngestor:
    ...
    def __init__(self, db: Session, service_role_key: str):
        ...
        # Optional: initialize a Supabase client here
        # self.supabase = create_supabase_client(service_role_key)
    ...
    def fetch_chapter_content(self, url: str) -> tuple[str, str]:
        try:
            ...
            for el in soup.select('h1, title'):
                ...
                if txt:
                    title = re.sub(r'[-|]爱下电子书.*$', '', txt).strip()
                    break
            ...
            if page_div:
                ...
                filtered = [
                    l for l in lines
                    if len(l) > 10 and not any(x in l for x in ['广告','ADVERTISEMENT','推广','SPONSORED'])
                ]
                content = "\n".join(filtered)
    ...
    def ingest_novel(self, url: str, limit: int = 5) -> dict:
        try:
            ...
            existing = self.db.query(Novel).filter(Novel.source_url == url).first()
            if existing:
                return {"status": "exists", "novel_id": existing.id, "message": "Novel exists"}
